...
def get_faces():
    ...
    total = dict_from_row(c.fetchone())['count']
...
def my_uploads():
    ...
    earn = dict_from_row(c.fetchone()).get('sum') or 0
    # 获取余额
    c.execute("SELECT balance FROM users WHERE id=%s", (u['id'],))
    balance = dict_from_row(c.fetchone())['balance']
    conn.close()
